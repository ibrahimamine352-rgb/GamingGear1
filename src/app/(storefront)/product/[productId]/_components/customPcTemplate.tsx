...
useEffect(() => {
        ...
        <div>  <div className="lg:grid lg:grid-cols-3 lg:items-start lg:gap-x-8">
            ...
            <div className="mt-10 md:mx-0 sm:mx-0 lg:mx-20 col-span-2 sm:mt-16 sm:px-0 lg:mt-0">
                <div>
                    ...
                    <Table className='border my-3 border-[hsl(var(--accent))] rounded-lg'>

                        <TableBody>
                            {TableData.map((invoice) => (
                                <TableRow className='hover:bg-[hsl(var(--accent)/0.10)] dark:hover:bg-[hsl(var(--accent)/0.12)] ' key={invoice.composant_name}>
  ...
  <div className="flex flex-col items-center text-center space-y-0 text-2xl mt-auto">
    {formattedproduct?.dicountPrice > 0 && (
      <span className="line-through mt-3 text-muted-foreground">
        <Currency value={formattedproduct?.dicountPrice} />
      </span>
    )}
    <b className="mt-2 text-[hsl(var(--accent))]">
