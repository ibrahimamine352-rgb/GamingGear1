...
if(prod.powersupplies.length>0){
  ...
  if(prodet.Power){
    data.push({id:i.toString(),name:"Puissance",value:prodet.Power.toString() + " watt"})
    i++
  }
...
if(prod.screens.length>0){
  ...
  if(prodet.Pouce){
    data.push({id:prodet.Pouce?.id ?? i.toString(), name: "Taille", value: (prodet.Pouce.name ? prodet.Pouce.name + '"' : 'zz')})
    i++
  }
  if(prodet.RefreshRate){
