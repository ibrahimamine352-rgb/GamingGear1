...
// ✅ helper: extract full UUID from the slug or fall back to slug
function extractIdFromParam(slugParam: string | undefined | null): string | null {
  ...
  // if it’s not a UUID, let Prisma try the raw slug as id
  return slugParam
}
...
export async function generateMetadata(
  ...
  try {
    ...
    if (!slug) {
      return {
        title: 'Produit | Gaming Gear TN',
        description: 'Produit gaming chez Gaming Gear TN.',
      }
    }

    const idCandidate = extractIdFromParam(slug)
    ...
    const ogImage =
      absoluteImageUrls[0] ?? `${baseUrl}/og/default-product.png` // make sure this file exists in /public/og/
...
const ProductPage: React.FC<ProductPageProps> = async ({ params }) => {
  try {
    ...
    // 🔥 If no product, show a simple 404-like page (no throw)
    if (!product) {
      return (
        <div className="min-h-screen flex items-center justify-center bg-background text-foreground">
          <div className="text-center">
            ...
            <p className="text-sm text-muted-foreground">
              We could not find any product for URL segment:{' '}
              <code>{slug}</code>
            </p>
          </div>
        </div>
    ...
    const suggestedProducts = await prismadb.product.findMany({
      where: { categoryId: product.category?.id ?? undefined },
    ...
    const formattedproducts: Product[] = suggestedProducts.map((item) => ({
      ...
      name: item.name,
      images: item.images ?? [],
      dicountPrice: item.dicountPrice ? Number(item.dicountPrice) : 0,
    ...
    const formattedproduct: Product = {
      ...
      dicountPrice: product.dicountPrice ? Number(product.dicountPrice) : 0,
    ...
    const productUrl = `https://gaminggeartn.tn/product/${slug}`
    const categoryName = product.category?.name || 'Produit Gaming'
    ...
    const jsonLd = {
      ...
      image:
        formattedproduct.images?.map((img) => img.url).filter(Boolean) ?? [],
    ...
    const dataa: Field[] = (formattedproduct.additionalDetails ?? []).map(
      (item) => ({
        id: item.id,
        name: item.name,
        value: item.value,
    ...
    const getData = (prod: typeof product): Field[] => {
      let i = 0
      if (prod && prod.Headset && prod.Headset.length > 0) {
      ...
      const mbs = await prismadb.motherboard.findMany({
        where: {
          products: {
            some: {
              id: {
                in: product.PreBuiltPcmodel.pcTemplate.motherBoardId.map(
                  (e) => e.productId
                ),
              },
            },
      ...
      const cpus = await prismadb.processor.findMany({
        where: {
          products: {
            some: {
              id: {
                in: product.PreBuiltPcmodel.pcTemplate.processorId.map(
                  (e) => e.productId
                ),
              },
            },
      ...
      const gpus = await prismadb.gpu.findMany({
        where: {
          products: {
            some: {
              id: {
                in: product.PreBuiltPcmodel.pcTemplate.graphicCardId.map(
                  (e) => e.productId
                ),
              },
            },
      ...
      const cases = await prismadb.pCcase.findMany({
        where: {
          product: {
            some: {
              id: {
                in: product.PreBuiltPcmodel.pcTemplate.caseId.map(
                  (e) => e.productId
                ),
              },
            },
      ...
      const power = await prismadb.powersupply.findMany({
        where: {
          products: {
            some: {
              id: {
                in: product.PreBuiltPcmodel.pcTemplate.powerSupplyId.map(
                  (e) => e.productId
                ),
              },
            },
      ...
      const coolings = await prismadb.cooling.findMany({
        where: {
          product: {
            some: {
              id: {
                in: product.PreBuiltPcmodel.pcTemplate.cooling.map(
                  (e) => e.productId
                ),
              },
            },
    ...
    if (product.PackProduct && product.PackProduct.length > 0) {
      return (
        <div className="bg-background min-h-screen">
          {/* JSON-LD for Google */}
          <script
    ...
    const showSale =
      !unavailable &&
      formattedproduct.dicountPrice &&
      formattedproduct.dicountPrice > 0
    ...
    // hard fallback so the server never crashes
    return (
      <div className="min-h-screen flex items-center justify-center bg-background text-foreground">
        <div className="text-center">
          <h1 className="text-2xl font-bold mb-2">
            Erreur de chargement produit
          </h1>
          <p className="text-sm text-muted-foreground">
            Un problème est survenu lors du chargement de ce produit. Veuillez
            réessayer plus tard.
          </p>
        </div>
      </div>
