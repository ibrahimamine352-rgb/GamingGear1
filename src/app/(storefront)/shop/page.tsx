...
const Home = async ({ searchParams }: { searchParams: { [key: string]: string | string[] | undefined } }) => {
  ...
  if (categoryy.length > 0) {
    ...
    // Try to find a real Category in DB
    ...
    if (cat) {
      ...
      else if (['power supplies', 'power-supplies', 'power supply', 'powersupply', 'powersupplies', 'psu', 'psus'].includes(k)) whereClause.powersupplies = { some: {} }
      ...
      // don’t poison query with a fake id if no match
    }

    // choose filter function branch
