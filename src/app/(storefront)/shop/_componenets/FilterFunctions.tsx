...
export const addcpuFitlters = (decoded: FilterList) => {
  ...
  if (decoded.cPUSupport?.length)
    and.push({ cpusupport: { name: { in: decoded.cPUSupport.map(i => i.searchKey) } } });

  if (decoded.processorModel?.length)
    and.push({ processorModel: { name: { in: decoded.processorModel.map(i => i.searchKey) } } });

  if (and.length) where.cpus = { some: { AND: and } };
