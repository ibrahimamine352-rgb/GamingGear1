...
export const cpusFilters = async () => {
  ...
  // Keep the key "processorModel" but show it to users as "Marque processeur"
  const processorModel: Filter = {
    title: "Marque processeur",
    list: processorModelRaw
      .map((row) => ({
        name: (row.name ?? "").toString().trim(), // expect AMD / Intel / etc.
        number: row._count?.processor ?? 0,
      }))
      .filter((x) => x.name.length > 0)
      .sort((a, b) => a.name.localeCompare(b.name, "fr", { numeric: true, sensitivity: "base" })),
  ...
  return {
    ...
    data: {
      //cPUSupport,
      //processorModel, // key unchanged for backend compatibility
    },
  };
};
