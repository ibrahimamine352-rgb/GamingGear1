...
export const Case = (props: {
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await fetch(
        `/api/pcCase/component?minDt=${priceFilter[0]}&maxDt=${
          priceFilter[1]
  ...
  const handleCheckboxChange = (
    filterKey: keyof checkItemGroupsCase,
    value: string
  ) => {
  ...
  const onPreview: MouseEventHandler<HTMLButtonElement> = (event) => {
    ...
    // previewModal.onOpen(...) if needed
  };
  const onAddToCart: MouseEventHandler<HTMLButtonElement> = (event) => {
    event.stopPropagation();
    // cart.addItem(...) if needed
  };
  ...
  const checkcompatibility = (product: Product) => {
    ...
    if (mb) {
      ...
      const PProfiles = props.profiles.filter((e) =>
        e.cases.find((ee) => ee.productId == product.id)
      );
  ...
  const dialogLayerRef = React.useRef<HTMLDivElement | null>(null);

  // Safely read description for the selected case (called processorId in props)
  const selectedCase = props.processorId;
  const selectedDesc = selectedCase?.description ?? "";
              ...
              <Card className="build-selector">
                ...
                <CardContent>
                  <div className="grid place-items-center h-24 leading-none text-[hsl(var(--accent))]">
                    <CaseIcon className="block w-24 h-24 md:w-28 md:h-28" />
                  </div>
                </CardContent>
              </Card>
            </button>
                    ...
                    <div className="build-container hover:bg-[#1a1a1a] transition flex md:flex-row flex-col justify-between group items-center rounded-xl space-x-3">
                      ...
                      <div className="flex-shrink-0 p-3">
                        <div className="text-center flex flex-col">
                          <div className="p-3">
                            <Currency value={selectedCase.price} />
                          </div>
                          <Button
                            className="mb-3"
                            disabled={loading}
                            ...
                            <CheckboxGroup
                              ...
                              onChange={(value) =>
                                handleCheckboxChange(
                                  filterKey as keyof checkItemGroupsCase,
                                  value
                                )
                              }
