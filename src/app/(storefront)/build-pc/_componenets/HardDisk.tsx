...
export const HardDisk = (props: {
  ...
  const handleCheckboxChange = (
    filterKey: keyof checkItemGroupsHardDisk,
    value: string
  ) => {
    setFilterList((prev) => {
  ...
  const checkcompatibility = (product: Product) => {
    ...
    if (mb) {
      ...
      const PProfiles = props.profiles.filter((e) =>
        e.disks.find((ee) => ee.Components.find((Xc) => Xc.productId == product.id))
      );
  ...
  const compatRows = [
    ...
    {
      key: "powerCompatibility",
      label: "Boîte d’alimentation compatibilité :",
    },
    { key: "caseCompatibility", label: "Case compatibility:" },
  ] as const;

  // ✅ Safe selected product fields
  const selectedDisk = props.processorId;
  const selectedDesc = selectedDisk?.description ?? "";
  const selectedImg =
    selectedDisk?.images?.[0]?.url || "/placeholder.png";
              ...
              <CardContent className="p-0 w-full h-full">
                {selectedDisk ? (
                  <div className="build-container hover:bg-[#1a1a1a] transition flex md:flex-row flex-col justify-between group items-center rounded-xl space-x-3">
                        ...
                        <div className="text-left grid text-xs max-w-screen-md mx-auto border border-border rounded mb-3 mr-3">
                          {compatRows.map(({ key, label }) => {
                            ...
                            return (
                              <div
                                key={key}
                                className="p-1 pl-3 border-b last:border-b-0 border-border hover:bg-muted hover:font-bold cursor-pointer"
                              >
                                <p
                                  className={`mb-1 ${
                    ...
                    <div className="flex-shrink-0 p-3">
                      <div className="text-center flex flex-col">
                        <div className="p-3">
                          <Currency value={selectedDisk.price} />
                        </div>
                        <Button
                          className="mb-3"
                          disabled={loading}
                  ...
                  >
                    ...
                    >
                      <g>
                        <path d="M41.9,21H23V2.1c0-0.6-0.5-1-1-1c-0.6,0-1,0.5-1,1V21H2.1c-0.6,0-1,0.5-1,1s0.5,1,1,1H21v18.8 c0,0.6,0.5,1,1,1c0.6,0,1-0.5,1-1V23h18.8c0.6,0,1-0.5,1-1S42.5,21,41.9,21z" />
                      </g>
                    </svg>
                  </div>
                )}
                            ...
                            <CheckboxGroup
                              ...
                              onChange={(value) =>
                                handleCheckboxChange(
                                  filterKey as keyof checkItemGroupsHardDisk,
                                  value
                                )
                              }
