...
export const GraphicCard = (props: {
  ...
  setProcessorId: (values: Product | undefined) => void; // GPU, but we keep your naming
  removeFeature: (featureName: string) => void;
  addFeature: (featureName: string, value: any) => void;
  selectedFeatures: SelectedFeatures;
  processorId: Product | undefined; // current selected GPU
  ...
  const [recommendedGpuMemorie, setRecommendedGpuMemorie] = useState<number | null>(null);
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await fetch(
        `/api/gpu/component?minDt=${priceFilter[0]}&maxDt=${
          priceFilter[1]
      ...
      try {
        ...
        if (reco && typeof reco.videoCardMemory === "number") {
          setRecommendedGpuMemorie(reco.videoCardMemory);
        } else {
          setRecommendedGpuMemorie(null);
        }
      } catch (err) {
  ...
  const handleCheckboxChange = (
    filterKey: keyof checkItemGroupsGPU,
    value: string
  ) => {
    setFilterList((prev) => {
      ...
      return { ...prev, [filterKey]: nextList };
    });
  };
  ...
  // currently not used; keep behavior same (always true)
  ...
  }, [currentPage, selectedSort, priceFilter, searchTerm, filterList, compatible, props.motherboardId?.id]);
  ...
  // Safely derive selected GPU fields to avoid .length on undefined
  const selectedGpu = props.processorId;
  const gpuDesc = selectedGpu?.description ?? "";
  const gpuImg = selectedGpu?.images?.[0]?.url || "/placeholder.png";
                ...
                {selectedGpu ? (
                  <div className="build-container hover:bg-[#1a1a1a] transition flex md:flex-row flex-col justify-between group items-center rounded-xl space-x-3">
                    ...
                    {props.selectedCompatibility ? (
                      <div className="w-4/5">
                        <div className="font-bold my-2 text-sm text-[#e6e8ee]">
                          Compatibility:
                        </div>
                    ...
                    <div className="flex-shrink-0 p-3">
                      <div className="text-center flex flex-col">
                        <div className="p-3">
                          <Currency value={selectedGpu.price} />
                        </div>
                        <Button
                          className="mb-3"
                          disabled={loading}
                          ...
                          onClick={() => {
                            ...
                            props.removeFeature("Video Card_Memory");
                          }}
                        >
                        ...
                        <Button
                          onClick={() => steOpenDialog(true)}
                          className="bg-[#00e0ff] text-black hover:bg-[#101218] border border-white"
                        >
                          {ui.builderBtnChange}
                        </Button>
                        <a
                          href={"product/" + selectedGpu.id}
                          className="underline mt-2 text-[#00e0ff]"
                          target="_blank"
                          rel="noreferrer"
                        >
          ...
          >
            ...
            <div className="flex-1 grid grid-cols-12 gap-4 px-4 py-4 overflow-hidden text-[13px]">
              ...
              <div className="col-span-12 md:col-span-4 lg:col-span-3 overflow-y-auto pr-1">
                ...
                {props.gpuBrand?.list && props.gpuBrand.list.length > 0 && (
                  <>
                    {props.motherboardId ? (
                      <div className="mt-2">
                    ...
                    {Object.entries(props).map(([filterKey, filter]) => {
                      if (
                        ["gpuBrand", "gpuArchBrand", "graphiccardName"].includes(
                          filterKey
                        )
                      ) {
                          ...
                          <CheckboxGroup
                            ...
                            onChange={(value) =>
                              handleCheckboxChange(
                                filterKey as keyof checkItemGroupsGPU,
                                value
                              )
                            }
                      ...
                      <>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                          {data.map((item, key) => {
                            ...
                            const isRecommended =
                              recommendedGpuMemorie !== null &&
                              gpuSize === recommendedGpuMemorie;
                                ...
                                <div className="w-full">
                                  ...
                                  <Button
                                    ...
                                    onClick={() => {
                                      ...
                                      const size = extractGpuSize(item.name);
                                      if (size !== null) {
                                        props.addFeature(
                                          "Video Card_Memory",
                                          size
