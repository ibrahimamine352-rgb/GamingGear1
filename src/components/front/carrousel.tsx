        ...
        {slides.map((each, index) => (
            ...
            style={{
              ...
              backgroundPosition: "center", // You can adjust this property based on your needs
                ...
                <div className='basis-2/3 p-28 dark:text-foreground max-sm:p-0 max-sm:flex max-sm:align-middle max-sm:content-center max-sm:flex-col '>
...
{each.Price.length > 0 ? (
  <p className='pb-2 font-semibold' style={{ color: each.PriceColor.length > 0 ? each.PriceColor : '' }}>
    {each.Price} TND
                    ...
                    {
                      each.url.length>0?<>
                      <div className='flex items-center'><Link  className='max-sm:w-full'    href={each.url}><Button  className='max-sm:w-full btn-neon' variant={"default"} >Shop</Button></Link> <Label className='pl-5'>{each.discount>0? <>{each.discount}</>:<></>} </Label></div> 
